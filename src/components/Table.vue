<template>
  ...
  >
  <div v-show="store.states.columns.length > 0">
    <div
      ref="wrapper"
    ...
    <div style="display: flex;padding: 10px 5px" v-if="addAble || deleteAble">
        <a href="javascript:void(0)" v-if="addAble" @click="addRow"  class="icon" title="新增行" :class="{addRow: true}"></a>&nbsp;
        <a href="javascript:void(0)" v-if="deleteAble" @click="deleteRow"  class="icon" title="删除行" :class="{deleteRow: true}"></a>
    </div>
    </div>
    <div
  ...
  methods: {
    fix_column(th) {
      this.columns.forEach((column) => {
        if (column.key === th.key) {
          if (column.fixed) {
            this.$set(column, "fixed", false);
          } else {
            this.$set(column, "fixed", true);
          }
        }
      })
    },
    addRow() {
      ...
      obj.willTableId = uuid.v1();
      this.addItem(obj)
      const Top = this.store.states.tableHeight - this.store.states.mainHeight > 0 ? this.store.states.tableHeight - this.store.states.mainHeight : 0;
      this.store.setScrollStatus(Top, this.store.states.tableBodyLeft);
    },
    deleteRow() {
      if (this.deleteSelections.length === 0) {
        message.warning('请勾选操作项');
        return;
      }
      const selectedIds = this.deleteSelections.map((item) => item.willTableId);
      this.removeItems("willTableId", selectedIds)
    ...
    selectionChange() {
      ...
      this.deleteSelections = selection
    ...
    delete_column(obj) {
      for (const v of this.data) {
        delete v[obj.col.key]
      }
      this.columns.splice(_.findIndex(this.columns, ['key', obj.col.key]), 1);
    },
    edit_column(obj) {
      const cIndex = obj.index
      const originalKey = this.columns[cIndex].key
      this.columns[cIndex].title = obj.label
      this.columns[cIndex].key = obj.key
      this.data.forEach((item) => {
        this.$set(item, obj.key, item[originalKey]);
    ...
    add_new_column(obj) {
      ...
      if (obj.type === "left") {
        this.columns.splice(cIndex, 0, onj);
      } else {
        this.columns.splice(cIndex + 1, 0, onj);
      }
      this.data.forEach((item) => {
    ...
    initColumns() {
      const { states } = this.store;
      const fixedArr = this.columns.filter((item) => item.fixed && item.key !== 'willTableId');
      const unFixedArr = this.columns.filter((item) => !item.fixed && item.key !== 'willTableId');
